...
const Dashboard = () => {
	...
	useEffect(() => {
		...
		const fetchConversations = async () => {
			console.log(loggedInUser?.id);
			const res = await fetch(`${baseAPI}/api/conversations/${loggedInUser?.id}`, {
				method: 'GET',
				headers: {
					'Content-Type': 'application/json',
				}
			});
			console.log("datyaayuaytytfythghgghkghgkhgkhugu");
	...
	useEffect(() => {
		const fetchUsers = async () => {
			const res = await fetch(`${baseAPI}/api/users/${user?.id}`, {
				method: 'GET',
				headers: {
					'Content-Type': 'application/json',
				}
	...
	const fetchMessages = async (conversationId, receiver) => {
		const res = await fetch(`${baseAPI}/api/message/${conversationId}?senderId=${user?.id}&&receiverId=${receiver?.receiverId}`, {
			method: 'GET',
			headers: {
				'Content-Type': 'application/json',
			}
	...
	const sendMessage = async (e) => {
		...
		const res = await fetch(`${baseAPI}/api/message`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
		...
		<div className='w-screen flex'>
			<div className='w-[25%] h-screen bg-secondary overflow-scroll'>
				<div className='flex items-center my-8 mx-14'>
					<div>
						<img  src={logo} width={80} height={75} className='border  border-primary p-[8px]  rounded-full' alt='logo' /></div>
					<div className='ml-8'>
									...
									return (
										<div className='flex items-center py-8 border-b border-b-gray-300'>
											<div className='cursor-pointer flex items-center' onClick={() => fetchMessages(conversationId, user)}>
												<div><img src={Img1} className="w-[60px] h-[60px] rounded-full p-[2px] border border-primary" alt='image 1' /></div>
												<div className='ml-6'>
								...
								return (
									<div className='flex items-center py-8 border-b border-b-gray-300'>
										<div className='cursor-pointer flex items-center' onClick={() => fetchMessages('new', user)}>
											<div><img src={Img1} alt='image' className="w-[60px] h-[60px] rounded-full p-[2px] border border-primary" /></div>
											<div className='ml-6'>
												<h3 className='text-lg font-semibold'>{user?.fullName}</h3>
												<p className='text-sm font-light text-gray-600'>{user?.email}</p>
											</div>
